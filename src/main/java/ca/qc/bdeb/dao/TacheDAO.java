...
public class TacheDAO implements ITacheDAO {
	...
	// Une méthode pour retourner une tâche (l’identifiant est un paramètre de cette méthode)
	@Override
	...
	//Une méthode pour retourner toutes les tâches d’un utilisateur donnée (l’identifiant de l’utilisateur est un 
	//paramètre de cette méthode) Une méthode pour retourner une tâche après sa suppression de la
	@Override
	public List<Tache> trouverTachesDuUtilisateur(Integer idUtilisateur) {
		...
		try {
			...
			if(rs.next()) {
				...
				tache.setTacheId(rs.getInt(3));
				tache.setDescription(rs.getString(4));
				tache.setDatetime(rs.getDate(5).toLocalDate());
				tache.setDuree(rs.getInt(6));
				tache.setUtilisateurId(rs.getInt(7));
	...
	//Une méthode pour retourner une tâche après sa suppression de la base de données (l’identifiant est un paramètre de cette méthode)
	@Override
